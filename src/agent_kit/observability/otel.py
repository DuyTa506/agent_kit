"""OpenTelemetry observer adapter (requires ``pip install 'agent-kit[otel]'``).

Maps AgentKit span hooks to OTel spans in a run → turn → (provider | tool)
...
class OpenTelemetryObserver(BaseObserver):
    """Maps AgentKit lifecycle hooks to OpenTelemetry spans.
    ...
    console exporter — no AgentKit changes required.
    """
    ...
    def on_run_start(self, info: RunInfo) -> None:
        ...
        span.set_attribute("agent_kit.run_id", info.run_id)
        span.set_attribute("agent_kit.session_id", info.session_id)
    ...
    def on_run_end(self, info: RunResultInfo) -> None:
        ...
        span.set_attribute("agent_kit.subtype", info.subtype)
        ...
        span.set_attribute("agent_kit.duration_ms", info.duration_ms)
    ...
    def on_turn_start(self, info: TurnInfo) -> None:
        ...
        span.set_attribute("agent_kit.run_id", info.run_id)
        span.set_attribute("agent_kit.turn_index", info.turn_index)
    ...
    def on_tool_start(self, info: ToolInfo) -> None:
        ...
        span.set_attribute("agent_kit.run_id", info.run_id)
        span.set_attribute("agent_kit.turn_index", info.turn_index)
        span.set_attribute("agent_kit.tool.name", info.tool_name)
        span.set_attribute("agent_kit.tool.use_id", info.tool_use_id)
    ...
    def on_tool_end(self, info: ToolResultInfo) -> None:
        ...
        if span is None:
            return
        span.set_attribute("agent_kit.tool.is_error", info.is_error)
        span.set_attribute("agent_kit.duration_ms", info.duration_ms)
        if info.is_error:
            span.set_status(StatusCode.ERROR)
        span.end()
